...
6. **Preventing Audio-Video Misalignment in Secondary Cuts / 防止二次切割的音视频对不齐**
   - For the first segment, if the video start point is later than the audio start point (e.g., due to pre-cutting with `ffmpeg -c copy`), skip using the `ss` parameter and start cutting directly from the video's beginning.
   - 对于首段视频，如果视频起始点晚于音频起始点（例如被 `ffmpeg -c copy` 预切割了一次），则不使用 `ss` 参数，直接从视频起始点开始切割。
   - Prevents transcription timeline offset.
   - 防止听写轴偏移。